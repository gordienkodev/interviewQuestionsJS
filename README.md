...
-1. Запрос (Request)
Этап запроса начинается, когда пользователь вводит URL страницы в адресную строку браузера, щелкает по ссылке, отправляет форму или выполняет любое действие, вызывающее запрос на сервер. Браузер отправляет HTTP-запрос на сервер для получения содержимого страницы.
-2. Загрузка (Loading)
На этапе загрузки браузер получает ответ от сервера, содержащий HTML-код, который составляет основу страницы. Этот HTML-код может также содержать ссылки на другие ресурсы, такие как CSS, JavaScript, изображения и т.д. Этот процесс включает несколько подэтапов:
    DNS Lookup: Определение IP-адреса сервера по доменному имени.
    TCP Handshake: Установление соединения с сервером.
    TLS Handshake: Если соединение защищено (HTTPS), установление защищенного соединения.
    Получение ресурсов: Загрузка HTML-документа и связанных ресурсов.
-3. Рендеринг (Rendering)
После получения HTML-кода страницы и всех связанных ресурсов (CSS, JavaScript, изображения) начинается рендеринг контента страницы. Этот процесс включает следующие шаги:
    ...
    Layout: Определение размеров и расположения каждого элемента на странице.
    Painting: Отрисовка элементов на экране.
-4. Интерактивность (Interactivity)
Когда страница полностью загружена и отрендерена, пользователь может взаимодействовать с ней. Это включает в себя щелчки мышью, ввод текста, отправку форм и другие действия. На этом этапе также могут выполняться JavaScript-скрипты, добавляющие динамическое поведение на страницу.
-5. Выгрузка (Unload)
Если пользователь переходит на другую страницу или закрывает текущую, страница проходит через этап выгрузки. На этом этапе браузер освобождает память, занятую ресурсами страницы, и сбрасывает ее состояние. Событие unload может использоваться для выполнения завершающих операций, таких как сохранение состояния или очистка ресурсов.

Дополнительные этапы (опционально)
    DOMContentLoaded: Событие, которое происходит, когда первоначальный HTML-документ полностью загружен и разобран, без ожидания завершения загрузки стилей, изображений и подкастов.
    Load: Событие, которое происходит, когда вся страница загружена, включая все зависимости, такие как стили и изображения.
    BeforeUnload: Событие, которое возникает перед выгрузкой страницы, позволяя выполнять действия, такие как предупреждение пользователя о несохраненных изменениях.

...
Стек (Stack): Хранит коллекцию элементов в линейном порядке, определяемом порядком операций. Этот порядок может быть "последний вошел, первый вышел" (LIFO) или "первый вошел, первый вышел" (FIFO).
...
Куча (Heap): Древовидная структура, в которой значение ключа каждого родительского узла больше или равно значению ключа любого из его дочерних узлов.
